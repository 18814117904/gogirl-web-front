                  ...
                  <div class="fixed-table-container" style="padding-bottom: 0px;">
                    ...
                    <div class="fixed-table-body" style="width: 100%;height: 250px;overflow: auto">
                      ...
                      <table id="roletable" class="table table-hover table-striped" style="margin-top: 0px;">
                        ...
                        <tbody>
                        <tr data-index="0" v-for="type in types" v-bind:key="type.id">
                          <td>{{type.name}}</td>
                          <td>{{type.createUser}}</td>
                          <td>{{type.createTime}}</td>
                          <td>{{type.updateTime}}</td>
                          <td>
                            <el-button type="danger" plain @click="deleteType(type.id)">删除</el-button>
                            <el-button type="primary" plain @click="updateType(type.id)">修改</el-button>
                          </td>
                        </tr>
                        </tbody>
                      </table>
  ...
  export default {
    ...
    methods: {
      ...
      getAllType: function (pageNum) {
        var url = this.$BASE_URL + 'type/queryTypeForPage/'
      ...
      deleteType: function (value) {
        this.$confirm('此操作将永久删除该类型, 是否继续?', '提示', {
          ...
          var url = this.$BASE_URL + 'type/deleteTypeById/'
      ...
      create: function () {
        this.$prompt('新增类型名称', '提示', {
          ...
          var url = this.$BASE_URL + 'type/addType/'
          var addTypeAxios = this.$axios.create({})
          addTypeAxios.get(url, {
            params: {
              name: value,
      ...
      updateType: function (no) {
        ...
        this.$prompt('修改类型名称', '提示', {
          ...
          var url = this.$BASE_URL + 'type/modifyType/'
